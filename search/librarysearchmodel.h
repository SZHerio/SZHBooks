...
class LibrarySearchModel final : public QAbstractListModel
{
    ...
    Q_PROPERTY(bool cancellationRequested READ cancellationRequested NOTIFY indexingChanged)
    Q_PROPERTY(int indexCompleted READ indexCompleted NOTIFY indexProgressChanged)
    Q_PROPERTY(qreal indexProgress READ indexProgress NOTIFY indexProgressChanged)
...
public:
    ...
    int indexCompleted() const;
    qreal indexProgress() const;
...
private:
    ...
    void updateIndexProgress(int completed, int total);
    void refreshResults();
